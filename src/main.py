...
async def process_fight_tweets(result_tweets, is_single_tweet, tweet, single_scorecard_media, twitter_client):
    ...
    if result_tweets['has_scorecard'] and result_tweets['has_result']:
        ...
        if scorecard_first_name == result_first_name:
            await process_scorecard_result(scorecard_id, scorecard_tweet, result_tweet, twitter_client, formatted_fights)
            
    elif is_single_tweet:
        new_tweets_found = True
        process_single_tweet(tweet, single_scorecard_media, formatted_fights)
...
async def process_scorecard_result(scorecard_id, scorecard_tweet, result_tweet, twitter_client, formatted_fights):
...
def process_single_tweet(tweet, single_scorecard_media, formatted_fights):
